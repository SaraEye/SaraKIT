...
#if HBI==I2C
     VPROC_DBG_PRINT(VPROC_DBG_LVL_ERR,"SSL SSL_port_open() starting I2C\n");
#else
    VPROC_DBG_PRINT(VPROC_DBG_LVL_ERR,"SSL SSL_port_open() starting SPI\n");
    printk("SSL SSL_port_open() starting SPI\n");
#endif
   /* open device port */
   if(hal_open(&(ssl_drv_priv.dev[i].pClient),pDevCfg)<0)
   {
     VPROC_DBG_PRINT(VPROC_DBG_LVL_ERR,"HAL Port open failed HBI=%d I2C=%d\n",HBI,I2C);
     return  SSL_STATUS_INTERNAL_ERR;
   }

   ssl_drv_priv.dev[i].inuse = TRUE;
   *pHandle = (ssl_port_handle_t)&(ssl_drv_priv.dev[i]);
printk("init:%d ssl:%d handle:%d \n",i,(ssl_port_handle_t)&(ssl_drv_priv.dev[i]),pHandle);
   VPROC_DBG_PRINT(VPROC_DBG_LVL_FUNC,"Exit...\n");
   return status;
}
